print("SeedForge 010 파일 실행 시작", flush=True)
...
print("SeedForge 010 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_010_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 010의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 010 매수 필터 팩터 계산 중...", flush=True)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_010.py 를 끝까지 실행하세요.", flush=True)
        return
...
def analyze_exits() -> None:
    print("SeedForge 010 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_010.csv"
    if not summary_path.exists():
        print("summary_seedforge_010.csv가 없습니다. 먼저 python -u seedforge_010.py 를 실행하세요.", flush=True)
        return
...
def main() -> None:
    ...
    print("SeedForge 010 RSI 다이버전스 매도규칙 정밀 테스트 시작", flush=True)
    ...
    print(
        ...
        "최우수 매수필터 고정 × 매도정책 정밀 시뮬레이션 시작",
        flush=True,
    )
    # 009에서 가장 해석 가능했던 매수 프리셋 하나만 고정한다.
    # 다른 조건들은 버리지 않고, 이 정밀 테스트가 통과 후보를 만들면 별도 011에서 확장한다.
    entry_presets = (
    ...
    summary.to_csv(output / "summary_seedforge_010.csv", index=False, encoding="utf-8-sig")
    period_summary = pd.DataFrame(period_summaries)
    period_summary.to_csv(output / "periods_seedforge_010.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_010.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_010.csv'}", flush=True)
    best = summary.iloc[0]
