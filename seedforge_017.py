print("SeedForge 017 파일 실행 시작", flush=True)
...
print("SeedForge 017 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_017_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 017의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 017 매수 필터 팩터 계산 중...", flush=True)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_017.py 를 끝까지 실행하세요.", flush=True)
        return
...
def add_validation_scores(summary: pd.DataFrame, period_summary: pd.DataFrame) -> pd.DataFrame:
    """전체 점수에 train/test 안정성 정보를 붙여 017 장기 미청산 MFE 반납 방지 후보를 정렬한다."""
...
def analyze_exits() -> None:
    print("SeedForge 017 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_017.csv"
    if not summary_path.exists():
        print("summary_seedforge_017.csv가 없습니다. 먼저 python -u seedforge_017.py 를 실행하세요.", flush=True)
        return
...
def audit_end_of_test() -> None:
    ...
    print("SeedForge 017 end_of_test 코호트 감사", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_017.csv"
    if not summary_path.exists():
        print("summary_seedforge_017.csv가 없습니다. 먼저 python -u seedforge_017.py 를 실행하세요.", flush=True)
    ...
    cohort_path = output / "exit_reason_cohorts_seedforge_017.csv"
    worst_path = output / "end_of_test_worst_seedforge_017.csv"
...
def main() -> None:
    ...
    print("SeedForge 017 장기 미청산 MFE 반납 방지 테스트 시작", flush=True)
    ...
    print(
        ...
        "016 감사 결과 기반 장기 미청산 MFE 반납 방지정책 paired 테스트 시작",
        flush=True,
    )
    # 016 감사에서 end_of_test loser도 평균 MFE가 50% 안팎까지 갔다가 손실로 끝났다.
    # 017은 일반 trailing이 아니라 오래 보유한 포지션의 수익 반납만 겨냥한다.
    entry_presets = (
    ...
    risk_policies = (
        "base",
        "stale_mfe40_loss_after730",
        "stale_mfe50_loss_after730",
        "stale_mfe50_to_10_after730",
        "stale_mfe50_giveback50_after730",
    )
    ...
    summary.to_csv(output / "summary_seedforge_017.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(output / "periods_seedforge_017.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_017.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_017.csv'}", flush=True)
    best = summary.iloc[0]
