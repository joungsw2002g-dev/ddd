print("SeedForge 016 파일 실행 시작", flush=True)
...
print("SeedForge 016 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_016_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 016의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 016 매수 필터 팩터 계산 중...", flush=True)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_016.py 를 끝까지 실행하세요.", flush=True)
        return
...
def add_validation_scores(summary: pd.DataFrame, period_summary: pd.DataFrame) -> pd.DataFrame:
    """전체 점수에 train/test 안정성 정보를 붙여 016 최종 후보와 end_of_test 감사를 정렬한다."""
...
def analyze_exits() -> None:
    print("SeedForge 016 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_016.csv"
    if not summary_path.exists():
        print("summary_seedforge_016.csv가 없습니다. 먼저 python -u seedforge_016.py 를 실행하세요.", flush=True)
        return
...
def audit_end_of_test() -> None:
    ...
    print("SeedForge 016 end_of_test 코호트 감사", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_016.csv"
    if not summary_path.exists():
        print("summary_seedforge_016.csv가 없습니다. 먼저 python -u seedforge_016.py 를 실행하세요.", flush=True)
    ...
    cohort_path = output / "exit_reason_cohorts_seedforge_016.csv"
    worst_path = output / "end_of_test_worst_seedforge_016.csv"
...
def main() -> None:
    ...
    print("SeedForge 016 end_of_test 코호트 감사 준비 실행 시작", flush=True)
    ...
    print(
        ...
        "015 최종 2후보 재실행 후 end_of_test 감사 파일 생성 준비",
        flush=True,
    )
    # 014 결과 MDD 보완책은 base를 이기지 못했다.
    # 016은 새 규칙을 더하지 않고 두 후보의 end_of_test 코호트 분석을 가능하게 한다.
    entry_presets = (
    ...
    summary.to_csv(output / "summary_seedforge_016.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(output / "periods_seedforge_016.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_016.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_016.csv'}", flush=True)
    best = summary.iloc[0]
