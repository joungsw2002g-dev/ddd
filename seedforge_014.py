print("SeedForge 014 파일 실행 시작", flush=True)
...
print("SeedForge 014 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_014_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 014의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 014 매수 필터 팩터 계산 중...", flush=True)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_014.py 를 끝까지 실행하세요.", flush=True)
        return
...
def add_validation_scores(summary: pd.DataFrame, period_summary: pd.DataFrame) -> pd.DataFrame:
    """전체 점수에 train/test 안정성 정보를 붙여 014 MDD 보완 후보 순위를 만든다."""
...
def analyze_exits() -> None:
    print("SeedForge 014 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_014.csv"
    if not summary_path.exists():
        print("summary_seedforge_014.csv가 없습니다. 먼저 python -u seedforge_014.py 를 실행하세요.", flush=True)
        return
...
def main() -> None:
    ...
    print("SeedForge 014 neutral_only MDD 보완 테스트 시작", flush=True)
    ...
    print(
        ...
        "013 1위 neutral_only 고정 × MDD 보완정책 시뮬레이션 시작",
        flush=True,
    )
    # 013 결과 neutral_only는 test CAGR/PF가 가장 좋았지만 전체 MDD가 컸다.
    # 014는 그 1위 구조를 고정하고 MDD를 줄이는 보완책만 paired 비교한다.
    entry_presets = (
    ...
    exit_policies = ("neutral_only",)
    risk_policies = (
        "base",
        "neutral_first_70_second_full",
        "neutral_first_50_second_full",
        "loss20_after252",
        "loss25_after252",
        "kospi_below_loss_exit",
        "trail30_after_mfe50",
        "trail35_after_mfe50",
    )
    ...
    summary.to_csv(output / "summary_seedforge_014.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(output / "periods_seedforge_014.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_014.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_014.csv'}", flush=True)
    best = summary.iloc[0]
