print("SeedForge 011 파일 실행 시작", flush=True)
...
print("SeedForge 011 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_011_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 011의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
    ...
    for i, date in enumerate(dates):
        ...
        for j in list(positions):
            ...
            if neutral_enabled and j in neutral_at.get(i, set()):
                neutral_profitable = np.isfinite(previous_close) and previous_close >= position.entry_price
                neutral_held_long_enough = (date - position.entry_date).days >= 252
                if exit_policy in {"neutral_profit_only", "neutral_plus_overbought_profit_neutral"}:
                    if neutral_profitable:
                        position.pending_exit = "neutral_divergence"
                elif exit_policy == "neutral_loss_delay_252":
                    if neutral_profitable or neutral_held_long_enough:
                        position.pending_exit = "neutral_divergence"
                else:
                    position.pending_exit = "neutral_divergence"
            if (
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 011 매수 필터 팩터 계산 중...", flush=True)
...
def run_one(
    ...
    equity, exposure, trades, counts = simulate(market, buys, overbought, neutral, exit_policy)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_011.py 를 끝까지 실행하세요.", flush=True)
        return
...
def add_validation_scores(summary: pd.DataFrame, period_summary: pd.DataFrame) -> pd.DataFrame:
    """전체 점수에 train/test 안정성 정보를 붙여 011 순위를 만든다."""
...
def analyze_exits() -> None:
    print("SeedForge 011 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_011.csv"
    if not summary_path.exists():
        print("summary_seedforge_011.csv가 없습니다. 먼저 python -u seedforge_011.py 를 실행하세요.", flush=True)
        return
    summary = pd.read_csv(summary_path).sort_values("score", ascending=False).head(5)
    for row in summary.itertuples(index=False):
        tag = f"p{row.preset}_x{row.exit_policy}"
...
def main() -> None:
    ...
    print("SeedForge 011 RSI 다이버전스 매도정책 확장 검증 시작", flush=True)
    ...
    print(
        ...
        "보류 매수조건 확장 × 후보 매도정책 시뮬레이션 시작",
        flush=True,
    )
    # 010에서 살아남은 매도정책 2개만 고정하고, 보류했던 매수 조건군을 다시 넓힌다.
    frequencies = ("weekly", "monthly")
    heat_policies = ("none", "r20_top5", "r60_top10", "r120_top10")
    crash_policies = ("r252_bottom10", "r252_bottom20")
    vol_policies = ("vol60_top20", "vol120_top20")
    value_policies = ("none", "dvchg_top10", "dvchg_top20")
    market_policies = ("none", "kospi_ma200")
    entry_presets = tuple(
        (
            f"{frequency[0]}_h{heat_policy}_c{crash_policy}_v{vol_policy}_d{value_policy}_m{market_policy}",
            frequency,
            heat_policy,
            crash_policy,
            vol_policy,
    ...
    total_combinations = len(entry_presets) * len(exit_policies)
    print("매수프리셋                     매도정책                   CAGR    MDD    노출   t(노출조정) 승률  손익비 거래PF 거래수 놓침")
    output = D / "results"
    ...
    for preset_name, frequency, heat_policy, crash_policy, vol_policy, value_policy, market_policy in entry_presets:
        for exit_policy in exit_policies:
            ...
            tag = f"p{preset_name}_x{exit_policy}"
            trades.to_json(
            ...
            print(
                ...
                f"{preset_name:<30} {exit_policy:<26} "
    ...
    summary.to_csv(output / "summary_seedforge_011.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(output / "periods_seedforge_011.csv", index=False, encoding="utf-8-sig")
    print("\n상위 5개 조합(검증점수순)", flush=True)
    for rank, row in enumerate(summary.head(5).itertuples(index=False), 1):
        print(
            f"#{rank} preset {row.preset} / exit {row.exit_policy} | "
    ...
    print(f"요약 저장: {output / 'summary_seedforge_011.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_011.csv'}", flush=True)
    best = summary.iloc[0]
