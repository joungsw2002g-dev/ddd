print("SeedForge 012 파일 실행 시작", flush=True)
...
print("SeedForge 012 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_012_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 012의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 012 매수 필터 팩터 계산 중...", flush=True)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_012.py 를 끝까지 실행하세요.", flush=True)
        return
...
def add_validation_scores(summary: pd.DataFrame, period_summary: pd.DataFrame) -> pd.DataFrame:
    """전체 점수에 train/test 안정성 정보를 붙여 012 순위를 만든다."""
...
def analyze_exits() -> None:
    print("SeedForge 012 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_012.csv"
    if not summary_path.exists():
        print("summary_seedforge_012.csv가 없습니다. 먼저 python -u seedforge_012.py 를 실행하세요.", flush=True)
        return
...
def main() -> None:
    ...
    print("SeedForge 012 RSI 다이버전스 장기 미청산 보완 테스트 시작", flush=True)
    ...
    print(
        ...
        "최종 매수조건 고정 × 장기 미청산 보완정책 시뮬레이션 시작",
        flush=True,
    )
    # 011에서 최종 생존한 매수/매도 구조 하나를 고정하고, 장기 미청산 보완책만 분해한다.
    entry_presets = (
    ...
    exit_policies = ("neutral_plus_overbought",)
    risk_policies = (
        "base",
        "hold24_loss_exit",
        "hold36_exit",
        "hold24_kospi_below_exit",
        "trail35_after_mfe50",
        "trail40_after_mfe70",
        "neutral_first_70_second_full",
        "neutral_loss_half",
    )
    ...
    summary.to_csv(output / "summary_seedforge_012.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(output / "periods_seedforge_012.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_012.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_012.csv'}", flush=True)
    best = summary.iloc[0]
