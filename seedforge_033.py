...
def build_oriented_library(
    ...
    raw, groups = search.build_factor_library(
        close, open_, volume, dates, flows, required_factors=set(directions)
    )
...
def completed_jobs(restart: bool) -> set[str]:
    if restart or not SUMMARY_OUTPUT.exists():
        return set()
    # The summary can contain more than 100,000 wide rows.  Loading every
    # column and converting it back to dictionaries roughly doubles the peak
    # memory before each checkpoint.  Resume only needs the identifiers.
    existing = pd.read_csv(SUMMARY_OUTPUT, usecols=["job_id"])
    return set(existing["job_id"].dropna().astype(str))


def save_summary(rows: list[dict[str, object]]) -> None:
    """Append the newly completed checkpoint rows without rebuilding history."""
    if not rows:
        return
    frame = pd.DataFrame.from_records(rows).drop_duplicates("job_id", keep="last")
    if SUMMARY_OUTPUT.exists():
        columns = pd.read_csv(SUMMARY_OUTPUT, nrows=0).columns.tolist()
        missing = [column for column in columns if column not in frame.columns]
        extra = [column for column in frame.columns if column not in columns]
        if missing or extra:
            raise RuntimeError(
                "033 checkpoint 열이 기존 summary와 다릅니다. "
                f"누락={missing}, 추가={extra}. --restart 전에 파일을 백업하세요."
            )
        frame.reindex(columns=columns).to_csv(
            SUMMARY_OUTPUT, mode="a", header=False, index=False, encoding="utf-8"
        )
    else:
        frame.to_csv(SUMMARY_OUTPUT, index=False, encoding="utf-8-sig")
    rows.clear()
...
def validate_resume_manifest(
    ...
    if mismatches:
        previous_finalists = int(previous.get("full_portfolio_finalists", 0))
        previous_combos = int(previous.get("all_proxy_combinations", 0))
        finalist_option = 0 if previous_finalists == previous_combos else previous_finalists
        resume_options = [
            f"--max-factors {int(previous.get('max_factors', DEFAULT_MAX_FACTORS))}",
            f"--full-finalists {finalist_option}",
        ]
        if previous.get("same_group_combinations"):
            resume_options.append("--same-group")
        if int(previous.get("single_factors", 0)) > len(singles) and not args.include_unstable_singles:
            resume_options.append("--include-unstable-singles")
        if args.max_jobs:
            resume_options.append(f"--max-jobs {args.max_jobs}")
        resume_command = "run_seedforge_033-1.bat " + " ".join(resume_options)
        raise RuntimeError(
            "기존 033 checkpoint와 현재 탐색 범위가 다릅니다. "
            "기존 작업을 이어가려면 이전 탐색 옵션을 사용하고, 완전히 새로 "
            "시작할 때만 --restart를 사용하세요. "
            f"차이: {mismatches}\n예상 재개 명령: {resume_command}"
        )
...
def main() -> int:
    ...
    done = completed_jobs(args.restart)
    rows: list[dict[str, object]] = []
    executed_this_run = 0
        ...
        for config in configs:
            ...
            if args.max_jobs and executed_this_run >= args.max_jobs:
                ...
                if completed < total_jobs:
                    ...
                    more_runs = math.ceil(remaining / args.max_jobs)
                    percent = completed / total_jobs * 100
                    print(
                        "이번 --max-jobs 한도에 도달했습니다. "
                        "같은 명령으로 재실행하면 이어집니다."
                    )
                    print(
                        f"[PAUSED] {completed:,}/{total_jobs:,} ({percent:.6f}%) 완료, "
                        f"{remaining:,}작업 남음; 현재 한도라면 약 {more_runs:,}회 추가 실행"
                    )
    ...
    ranking = ["train_selection_score", "robust_train_calmar"]
    # The complete CSV is already the durable result.  Read only the columns
    # needed for the final console table instead of materialising all columns.
    result_frame = pd.read_csv(
        SUMMARY_OUTPUT, usecols=list(dict.fromkeys(display + ranking))
    ).sort_values(
