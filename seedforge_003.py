print("SeedForge 003 파일 실행 시작", flush=True)
...
print("SeedForge 003 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_003_prepared.pkl"
...
def run_one(
    ...
    volume_factor: float,
    use_relative_strength: bool,
) -> tuple[dict[str, float], pd.DataFrame, pd.Series, dict[str, int]]:
    ...
    return metrics, trades, equity, counts
...
def main() -> None:
    ...
    print("SeedForge 003 RSI 다이버전스 백테스트 시작", flush=True)
        ...
        for wait in (5, 10, 15):
            for volume_factor in (1.0, 1.2):
                for use_rs in (False, True):
                    ...
                    metrics, trades, equity, counts = run_one(
                        market,
                        features,
                        overbought,
                        neutral,
    ...
    summary.to_csv(output / "summary_seedforge_003.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_003.csv'}", flush=True)
