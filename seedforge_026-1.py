...
BUILD = "026.1.1-20260807-no-candidate-r1"
...
def main() -> None:
    ...
    print("\nDART 단독 train 결과")
    dart_columns = [
        "factor", "direction", "train_ic", "train_ic_2014_2017",
    ...
    print(dart_report[dart_columns].to_string(index=False))
    ...
    if dart_selected.empty or technical_selected.empty:
        reason = (
            "DART train-only 통과 후보 없음" if dart_selected.empty
            else "기술 train-only 통과 후보 없음"
        )
        empty_columns = [
            "factor", "factor_groups", "factor_count", "train_ic",
            "train_ic_2014_2017", "train_ic_2018_2021", "train_spread",
            "train_selected_return", "opened_2022_2026_ic",
            "opened_2022_2026_selected_return", "advance_to_combo",
            "selected_on_train_only",
        ]
        pd.DataFrame(columns=empty_columns).to_csv(
            RESULTS / "factor_combos_seedforge_026.csv", index=False, encoding="utf-8-sig"
        )
        print(f"\n조합 생성 생략: {reason}")
        print("사전등록 기준을 실행 후 완화하지 않습니다. DART 결과 CSV를 보존하고 026을 FAIL로 종료합니다.")
