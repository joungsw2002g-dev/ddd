print("SeedForge 019 파일 실행 시작", flush=True)
...
print("SeedForge 019 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_019_prepared.pkl"
PERIODS = (
    ("2014_2019", "2014-05-12", "2019-12-31"),
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 019의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 019 매수 필터 팩터 계산 중...", flush=True)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_019.py 를 끝까지 실행하세요.", flush=True)
        return
...
def add_validation_scores(summary: pd.DataFrame, period_summary: pd.DataFrame) -> pd.DataFrame:
    """전체 점수에 train/test 안정성 정보를 붙여 019 최종 2후보를 정렬한다."""
...
def analyze_exits() -> None:
    print("SeedForge 019 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_019.csv"
    if not summary_path.exists():
        print("summary_seedforge_019.csv가 없습니다. 먼저 python -u seedforge_019.py 를 실행하세요.", flush=True)
        return
...
def audit_end_of_test() -> None:
    ...
    print("SeedForge 019 end_of_test 코호트 감사", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_019.csv"
    if not summary_path.exists():
        print("summary_seedforge_019.csv가 없습니다. 먼저 python -u seedforge_019.py 를 실행하세요.", flush=True)
    ...
    cohort_path = output / "exit_reason_cohorts_seedforge_019.csv"
    worst_path = output / "end_of_test_worst_seedforge_019.csv"
...
def audit_entry_side(market: SimulationInputs, factors: EntryFactors) -> None:
    ...
    print("SeedForge 019 진입조건 코호트 감사", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_019.csv"
    if not summary_path.exists():
        print("summary_seedforge_019.csv가 없습니다. 먼저 python -u seedforge_019.py 를 실행하세요.", flush=True)
    ...
    audit_path = output / "entry_side_audit_seedforge_019.csv"
    cohort_path = output / "entry_side_cohorts_seedforge_019.csv"
    audit.to_csv(audit_path, index=False, encoding="utf-8-sig")
    if audit.empty:
        print(f"진입조건 감사 저장: {audit_path} (빈 파일)", flush=True)
        return
    cohort = audit.groupby(["candidate", "exit_reason"], dropna=False).agg(
        count=("return", "size"),
        avg_return=("return", "mean"),
        win_rate=("return", lambda value: (value > 0).mean()),
        avg_holding_days=("holding_days", "mean"),
...
def main() -> None:
    ...
    if "--audit-entry" in sys.argv:
        ...
        return
    refresh_cache = "--refresh-cache" in sys.argv
    print("SeedForge 019 진입 품질 필터 검증 시작", flush=True)
    ...
    print(
        ...
        "018 진입조건 감사 결과 기반 ret120/ret252 진입 품질 필터 테스트 시작",
        flush=True,
    )
    # 018 감사에서 end_of_test 장기 패자는 정상 청산군보다 ret120/ret252가 낮았다.
    # 019는 매도규칙을 그대로 두고, 진입 당시 중기/장기 모멘텀 품질 필터만 검증한다.
    entry_presets = (
        ...
        ("m_hr120_pos_cr252_bottom20_vvol60_top20_ddvchg_top20_mkospi_ma200", "monthly", "r120_positive", "r252_bottom20", "vol60_top20", "dvchg_top20", "kospi_ma200"),
        ("m_hr252_pos_cr252_bottom20_vvol60_top20_ddvchg_top20_mkospi_ma200", "monthly", "r252_positive", "r252_bottom20", "vol60_top20", "dvchg_top20", "kospi_ma200"),
        ...
        ("m_hr120_top60_cr252_bottom20_vvol60_top20_ddvchg_top20_mkospi_ma200", "monthly", "r120_top60", "r252_bottom20", "vol60_top20", "dvchg_top20", "kospi_ma200"),
    )
    exit_policies = (
        "neutral_only",
        "neutral_plus_overbought",
    ...
    summary.to_csv(output / "summary_seedforge_019.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(output / "periods_seedforge_019.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_019.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_019.csv'}", flush=True)
    best = summary.iloc[0]
