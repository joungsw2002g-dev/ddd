...
BUILD = "025.4-20260807"
TOP_COMBOS = 10


def main() -> None:
    for required in (COMBOS, SCREEN, BENCHMARK):
    ...
    for combo_row in combo_report.itertuples(index=False):
        ...
        for market_policy, market_ok in (("none", None), ("kospi_ma200", kospi_ok)):
            ...
            metrics = core.performance(
                equity, exposure, kospi.pct_change(fill_method=None).fillna(0), trades
            )
            row = {
    ...
    summary = add_objective_gates(summary, kospi)
