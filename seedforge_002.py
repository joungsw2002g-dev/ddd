print("SeedForge 002 파일 실행 시작", flush=True)
...
print("SeedForge 002 모듈 로딩 중...", flush=True)
...
def main() -> None:
    ...
    print("SeedForge 002 RSI 다이버전스 백테스트 시작", flush=True)
    ...
    print(
        f"대상 {len(features):,}종목, 전처리 {perf_counter() - started:.1f}초, "
        "조합별 신호/시뮬레이션 시작",
        flush=True,
    )
    print("RSI 대기 거래량 RS  CAGR    MDD    노출   t(노출조정) 승률  손익비 거래PF 거래수 놓침", flush=True)
    output = D / "results"
