print("SeedForge 004 파일 실행 시작", flush=True)
...
print("SeedForge 004 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_004_prepared.pkl"
PERIODS = (
...
def main() -> None:
    ...
    print("SeedForge 004 RSI 다이버전스 백테스트 시작", flush=True)
    ...
    summary.to_csv(output / "summary_seedforge_004.csv", index=False, encoding="utf-8-sig")
    period_summary = pd.DataFrame(period_summaries)
    period_summary.to_csv(output / "periods_seedforge_004.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_004.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_004.csv'}", flush=True)
    best = summary.iloc[0]
