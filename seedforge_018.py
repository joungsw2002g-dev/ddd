print("SeedForge 018 파일 실행 시작", flush=True)
...
print("SeedForge 018 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_018_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 018의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 018 매수 필터 팩터 계산 중...", flush=True)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_018.py 를 끝까지 실행하세요.", flush=True)
        return
...
def add_validation_scores(summary: pd.DataFrame, period_summary: pd.DataFrame) -> pd.DataFrame:
    """전체 점수에 train/test 안정성 정보를 붙여 018 최종 2후보를 정렬한다."""
...
def analyze_exits() -> None:
    print("SeedForge 018 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_018.csv"
    if not summary_path.exists():
        print("summary_seedforge_018.csv가 없습니다. 먼저 python -u seedforge_018.py 를 실행하세요.", flush=True)
        return
...
def audit_end_of_test() -> None:
    ...
    print("SeedForge 018 end_of_test 코호트 감사", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_018.csv"
    if not summary_path.exists():
        print("summary_seedforge_018.csv가 없습니다. 먼저 python -u seedforge_018.py 를 실행하세요.", flush=True)
    ...
    cohort_path = output / "exit_reason_cohorts_seedforge_018.csv"
    worst_path = output / "end_of_test_worst_seedforge_018.csv"
...
def audit_entry_side(market: SimulationInputs, factors: EntryFactors) -> None:
    ...
    print("SeedForge 018 진입조건 코호트 감사", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_018.csv"
    if not summary_path.exists():
        print("summary_seedforge_018.csv가 없습니다. 먼저 python -u seedforge_018.py 를 실행하세요.", flush=True)
    ...
    audit_path = output / "entry_side_audit_seedforge_018.csv"
    cohort_path = output / "entry_side_cohorts_seedforge_018.csv"
...
def main() -> None:
    ...
    if "--audit-entry" in sys.argv:
        ...
        return
    refresh_cache = "--refresh-cache" in sys.argv
    print("SeedForge 018 진입조건 감사 테스트 시작", flush=True)
    ...
    print(
        ...
        "017에서 매도 보완정책 실패 확인 후 end_of_test 진입조건 감사 시작",
        flush=True,
    )
    # 017에서 stale/MFE 반납 방지 매도정책은 base 후보를 이기지 못했다.
    # 018은 매도규칙 추가를 멈추고, end_of_test 장기 패자의 진입 당시 팩터를 감사한다.
    entry_presets = (
    ...
    summary.to_csv(output / "summary_seedforge_018.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(output / "periods_seedforge_018.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_018.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_018.csv'}", flush=True)
    best = summary.iloc[0]
