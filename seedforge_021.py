print("SeedForge 021 파일 실행 시작", flush=True)
...
print("SeedForge 021 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_021_prepared.pkl"
PERIODS = (
    ("2014_2018", "2014-05-12", "2018-12-31"),
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 021의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def simulate(
    ...
    for i, date in enumerate(dates):
        ...
        for j in list(positions):
            ...
            if (
                position.pending_exit is None
                and risk_policy == "stale_mfe40_loss_after730"
                and holding_days >= 730
                and position.mfe >= 0.40
                and np.isfinite(previous_return)
                and previous_return < 0
            ):
                position.pending_exit = "stale_mfe40_loss_after730"
            if (
                position.pending_exit is None
                and risk_policy == "stale_mfe50_loss_after730"
                and holding_days >= 730
                and position.mfe >= 0.50
                and np.isfinite(previous_return)
                and previous_return < 0
            ):
                position.pending_exit = "stale_mfe50_loss_after730"
            if (
                position.pending_exit is None
                and risk_policy == "stale_mfe50_to_10_after730"
                and holding_days >= 730
                and position.mfe >= 0.50
                and np.isfinite(previous_return)
                and previous_return <= 0.10
            ):
                position.pending_exit = "stale_mfe50_to_10_after730"
            if (
                position.pending_exit is None
                and risk_policy == "stale_mfe50_giveback50_after730"
                and holding_days >= 730
                and position.mfe >= 0.50
                and np.isfinite(previous_return)
                and previous_return <= position.mfe - 0.50
            ):
                position.pending_exit = "stale_mfe50_giveback50_after730"
            if (
                position.pending_exit is None
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 021 매수 필터 팩터 계산 중...", flush=True)
...
def build_rank_signals(
    ...
    heat_map = {
        ...
        "r120_top60": (factors.ret120, 0.40),
    }
    crash_map = {
        "none": None,
    ...
    for day in range(start, len(market.dates) - 1):
        ...
        if market_policy == "kospi_ret60" and not factors.kospi_ret60_positive[day]:
            continue
        if market_policy == "kospi_ma200_ret60" and not (factors.kospi_above_ma200[day] and factors.kospi_ret60_positive[day]):
            continue
        mask = np.isfinite(market.close[day]) & market.entry_eligible[day]
        if heat_policy == "r120_positive":
            mask &= np.isfinite(factors.ret120[day]) & (factors.ret120[day] > 0.0)
        elif heat_policy == "r252_positive":
            mask &= np.isfinite(factors.ret252[day]) & (factors.ret252[day] > 0.0)
        elif heat_policy == "r120_r252_positive":
            mask &= (
                np.isfinite(factors.ret120[day])
                & np.isfinite(factors.ret252[day])
                & (factors.ret120[day] > 0.0)
                & (factors.ret252[day] > 0.0)
            )
        elif heat_map[heat_policy] is not None:
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_021.py 를 끝까지 실행하세요.", flush=True)
        return
...
def add_validation_scores(summary: pd.DataFrame, period_summary: pd.DataFrame) -> pd.DataFrame:
    """검증점수와 별도로 MDD 게이트를 강하게 반영한 robust_score를 계산한다."""
    ...
    period_metrics = period_summary.loc[
        period_summary["period"].isin(["2018_2020", "train_2014_2021", "test_2022_2026", "2024_2026"]),
    ...
    full_mdd_penalty = np.maximum(0.0, (-0.30 - merged["mdd"].fillna(-1.0)) * 40.0)
    stress_mdd = merged["mdd_2018_2020"].fillna(-1.0)
    stress_mdd_penalty = np.maximum(0.0, (-0.30 - stress_mdd) * 30.0)
    weak_train_penalty = np.maximum(0.0, (0.04 - merged["cagr_train_2014_2021"].fillna(-1.0)) * 50.0)
    merged["passes_mdd_gate"] = (merged["mdd"] >= -0.30) & (stress_mdd >= -0.30) & (test_mdd >= -0.22)
    merged["robust_score"] = merged["validation_score"] - full_mdd_penalty - stress_mdd_penalty - weak_train_penalty
    return merged

def analyze_exits() -> None:
    print("SeedForge 021 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_021.csv"
    if not summary_path.exists():
        print("summary_seedforge_021.csv가 없습니다. 먼저 python -u seedforge_021.py 를 실행하세요.", flush=True)
        return
...
def audit_end_of_test() -> None:
    """최종 후보별 end_of_test loser와 정상 다이버전스 청산 코호트를 CSV로 분해한다."""
    print("SeedForge 021 end_of_test 코호트 감사", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_021.csv"
    if not summary_path.exists():
        print("summary_seedforge_021.csv가 없습니다. 먼저 python -u seedforge_021.py 를 실행하세요.", flush=True)
    ...
    for row in summary.itertuples(index=False):
        risk = getattr(row, "risk_policy", "base")
        tag = f"p{row.preset}_x{row.exit_policy}_r{risk}"
        path = output / f"trades_{tag}.json"
        if not path.exists():
            print(f"거래원장 없음: {path}", flush=True)
            continue
        trades = pd.read_json(path)
        if trades.empty:
            continue
        trades["candidate"] = f"{row.exit_policy}/{risk}"
        trades["preset"] = row.preset
        grouped = trades.groupby("exit_reason", dropna=False).agg(
            count=("return", "size"),
            avg_return=("return", "mean"),
            win_rate=("return", lambda value: (value > 0).mean()),
            avg_holding_days=("holding_days", "mean"),
            avg_mae=("mae", "mean"),
            avg_mfe=("mfe", "mean"),
        ).reset_index()
        for cohort in grouped.to_dict("records"):
            cohort.update(candidate=f"{row.exit_policy}/{risk}", preset=row.preset)
            cohorts.append(cohort)
        end_trades = trades.loc[trades["exit_reason"] == "end_of_test"].copy()
        if not end_trades.empty:
            for item in end_trades.sort_values("return").head(20).to_dict("records"):
                worst_rows.append(item)
    cohort_frame = pd.DataFrame(cohorts)
    worst_frame = pd.DataFrame(worst_rows)
    cohort_path = output / "exit_reason_cohorts_seedforge_021.csv"
    worst_path = output / "end_of_test_worst_seedforge_021.csv"
    cohort_frame.to_csv(cohort_path, index=False, encoding="utf-8-sig")
    worst_frame.to_csv(worst_path, index=False, encoding="utf-8-sig")
    print(f"청산사유 코호트 저장: {cohort_path}", flush=True)
    print(f"end_of_test 최악 거래 저장: {worst_path}", flush=True)
    if not cohort_frame.empty:
        print(cohort_frame.to_string(index=False, float_format=lambda value: f"{value:,.4f}"), flush=True)



def _normalize_ticker(value: object) -> str:
    """JSON/CSV 재로딩 중 숫자로 해석된 6자리 종목코드를 원래 문자열로 복구한다."""
    text = str(value).strip()
    if text.endswith(".0") and text[:-2].isdigit():
        text = text[:-2]
    return text.zfill(6) if text.isdigit() and len(text) <= 6 else text

def _lookup_entry_value(values: np.ndarray, date_index: int, column: int) -> float:
    """2차원 종목 팩터 또는 1차원 시장 팩터에서 진입일 값을 안전하게 읽는다."""
    if values.ndim == 1:
        value = values[date_index]
    else:
        value = values[date_index, column]
    return float(value) if np.isfinite(value) else float("nan")


def audit_entry_side(market: SimulationInputs, factors: EntryFactors) -> None:
    """end_of_test 장기 패자의 진입 당시 팩터가 정상 청산군과 다른지 CSV로 감사한다."""
    print("SeedForge 021 진입조건 코호트 감사", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_021.csv"
    if not summary_path.exists():
        print("summary_seedforge_021.csv가 없습니다. 먼저 python -u seedforge_021.py 를 실행하세요.", flush=True)
    ...
    sort_column = "validation_score" if "validation_score" in summary.columns else "score"
    summary = summary.sort_values(sort_column, ascending=False)
    ticker_to_column = {ticker: column for column, ticker in enumerate(market.tickers)}
    ...
    for row in summary.itertuples(index=False):
        ...
        if not path.exists():
            print(f"거래원장 없음: {path}", flush=True)
            continue
        raw_trades = json.loads(path.read_text(encoding="utf-8"))
        if not raw_trades:
            continue
        for trade in raw_trades:
            ticker = _normalize_ticker(trade.get("ticker", ""))
            column = ticker_to_column.get(ticker)
            if column is None:
                continue
            entry_date = pd.Timestamp(trade["entry_date"])
            date_index = market.dates.get_indexer([entry_date])[0]
            if date_index < 0:
                continue
            record = dict(trade)
            record.update(
                candidate=f"{row.exit_policy}/{risk}",
                preset=row.preset,
                risk_policy=risk,
                entry_ret20=_lookup_entry_value(factors.ret20, date_index, column),
                entry_ret60=_lookup_entry_value(factors.ret60, date_index, column),
                entry_ret120=_lookup_entry_value(factors.ret120, date_index, column),
                entry_ret252=_lookup_entry_value(factors.ret252, date_index, column),
                entry_vol60=_lookup_entry_value(factors.vol60, date_index, column),
                entry_vol120=_lookup_entry_value(factors.vol120, date_index, column),
                entry_dollar_value_change=_lookup_entry_value(factors.dollar_value_change, date_index, column),
                entry_kospi_above_ma200=_lookup_entry_value(factors.kospi_above_ma200, date_index, column),
                entry_kospi_ret60_positive=_lookup_entry_value(factors.kospi_ret60_positive, date_index, column),
                is_end_of_test=trade.get("exit_reason") == "end_of_test",
                is_bad_end_of_test=trade.get("exit_reason") == "end_of_test" and float(trade.get("return", 0.0)) < 0.0,
            )
            enriched.append(record)
    audit = pd.DataFrame(enriched)
    audit_path = output / "entry_side_audit_seedforge_021.csv"
    cohort_path = output / "entry_side_cohorts_seedforge_021.csv"
    audit.to_csv(audit_path, index=False, encoding="utf-8-sig")
    if audit.empty:
        print(f"진입조건 감사 저장: {audit_path} (빈 파일)", flush=True)
        return
    cohort = audit.groupby(["preset", "candidate", "exit_reason"], dropna=False).agg(
        count=("return", "size"),
        avg_return=("return", "mean"),
        win_rate=("return", lambda value: (value > 0).mean()),
        avg_holding_days=("holding_days", "mean"),
        avg_mae=("mae", "mean"),
        avg_mfe=("mfe", "mean"),
        avg_entry_ret20=("entry_ret20", "mean"),
        avg_entry_ret60=("entry_ret60", "mean"),
        avg_entry_ret120=("entry_ret120", "mean"),
        avg_entry_ret252=("entry_ret252", "mean"),
        avg_entry_vol60=("entry_vol60", "mean"),
        avg_entry_vol120=("entry_vol120", "mean"),
        avg_entry_dollar_value_change=("entry_dollar_value_change", "mean"),
        avg_entry_kospi_above_ma200=("entry_kospi_above_ma200", "mean"),
        avg_entry_kospi_ret60_positive=("entry_kospi_ret60_positive", "mean"),
    ).reset_index()
    cohort.to_csv(cohort_path, index=False, encoding="utf-8-sig")
    print(f"진입조건 감사 저장: {audit_path}", flush=True)
    print(f"진입조건 코호트 저장: {cohort_path}", flush=True)
    print(cohort.to_string(index=False, float_format=lambda value: f"{value:,.4f}"), flush=True)
...
def main() -> None:
    ...
    if "--analyze-exits" in sys.argv:
        analyze_exits()
        return
    if "--audit-end" in sys.argv:
        audit_end_of_test()
        return
    if "--audit-entry" in sys.argv:
        refresh_cache = "--refresh-cache" in sys.argv
        market, _features, _overbought, _neutral, kospi_close = load_or_prepare(refresh_cache)
        factors = prepare_entry_factors(market, kospi_close)
        audit_entry_side(market, factors)
        return
    refresh_cache = "--refresh-cache" in sys.argv
    print("SeedForge 021 robust finalist 게이트 검증 시작", flush=True)
    ...
    print(
        ...
        "019 결과 기반 r120/r252 품질 후보의 시장국면 MDD 완화 테스트 시작",
        flush=True,
    )
    # 019에서 r120/r252 양수 필터는 test CAGR과 end_of_test 손실을 개선했지만 전체 MDD가 악화됐다.
    # 021은 매도규칙을 그대로 두고 시장국면 필터로 2018~2021 MDD가 줄어드는지만 검증한다.
    entry_presets = (
        ...
        ("m_hr120r252_pos_cr252_bottom20_vvol60_top20_ddvchg_top20_mnone", "monthly", "r120_r252_positive", "r252_bottom20", "vol60_top20", "dvchg_top20", "none"),
        ("m_hr120r252_pos_cr252_bottom20_vvol60_top20_ddvchg_top20_mkospi_ma200", "monthly", "r120_r252_positive", "r252_bottom20", "vol60_top20", "dvchg_top20", "kospi_ma200"),
        ("m_hr120r252_pos_cr252_bottom20_vvol60_top20_ddvchg_top20_mkospi_ret60", "monthly", "r120_r252_positive", "r252_bottom20", "vol60_top20", "dvchg_top20", "kospi_ret60"),
        ("m_hr120r252_pos_cr252_bottom20_vvol60_top20_ddvchg_top20_mkospi_ma200_ret60", "monthly", "r120_r252_positive", "r252_bottom20", "vol60_top20", "dvchg_top20", "kospi_ma200_ret60"),
    )
    exit_policies = (
        "neutral_only",
        "neutral_plus_overbought",
    ...
    summary = add_validation_scores(summary, period_summary).sort_values("robust_score", ascending=False)
    summary.to_csv(output / "summary_seedforge_021.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(output / "periods_seedforge_021.csv", index=False, encoding="utf-8-sig")
    print("\n상위 5개 조합(robust_score순: MDD 게이트 반영)", flush=True)
    for rank, row in enumerate(summary.head(5).itertuples(index=False), 1):
        print(
            ...
            f"robust {row.robust_score:.2f}, MDD게이트 {row.passes_mdd_gate}",
            flush=True,
        )
    print(f"요약 저장: {output / 'summary_seedforge_021.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_021.csv'}", flush=True)
    best = summary.iloc[0]
    ...
    print("\nend_of_test 코호트 감사 자동 실행", flush=True)
    audit_end_of_test()
    print("\n진입조건 코호트 감사 자동 실행", flush=True)
    audit_entry_side(market, factors)
