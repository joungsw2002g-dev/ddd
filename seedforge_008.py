print("SeedForge 008 파일 실행 시작", flush=True)
...
print("SeedForge 008 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_008_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 008의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
    ...
    for i, date in enumerate(dates):
        ...
        for j in list(positions):
            ...
            if closed:
                ...
                del positions[j]
                if reason == "price_stop" and stop_cooldown_days > 0:
                    cooldown_until[j] = i + stop_cooldown_days
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 008 매수 필터 팩터 계산 중...", flush=True)
...
def run_one(
    ...
    equity, exposure, trades, counts = simulate(market, buys, overbought, neutral, None, 0)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_008.py 를 끝까지 실행하세요.", flush=True)
        return
...
def analyze_exits() -> None:
    print("SeedForge 008 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_008.csv"
    if not summary_path.exists():
        print("summary_seedforge_008.csv가 없습니다. 먼저 python -u seedforge_008.py 를 실행하세요.", flush=True)
        return
    summary = pd.read_csv(summary_path).sort_values("score", ascending=False).head(5)
    for row in summary.itertuples(index=False):
        tag = (
            f"f{row.frequency}_h{row.heat_policy}_c{row.crash_policy}_"
            f"v{row.vol_policy}_d{row.value_policy}_m{row.market_policy}"
        )
...
def main() -> None:
    ...
    print("SeedForge 008 RSI 다이버전스 백테스트 시작", flush=True)
    ...
    heat_policies = ("none", "r20_top5", "r60_top10", "r120_top10")
    crash_policies = ("none", "r252_bottom10", "r252_bottom20")
    vol_policies = ("none", "vol60_top20", "vol120_top20")
    value_policies = ("none", "dvchg_top10", "dvchg_top20")
    market_policies = ("none", "kospi_ma200", "kospi_ret60")
    total_combinations = (
        len(frequencies)
        * len(heat_policies)
        * len(crash_policies)
    ...
    print("주기    과열제외    붕괴제외       변동성       거래대금      시장        CAGR    MDD    노출   t(노출조정) 승률  손익비 거래PF 거래수 놓침")
    output = D / "results"
    ...
    for frequency in frequencies:
        for heat_policy in heat_policies:
            for crash_policy in crash_policies:
                for vol_policy in vol_policies:
                    for value_policy in value_policies:
                        for market_policy in market_policies:
                            combination += 1
                            ...
                            tag = (
                            f"f{frequency}_h{heat_policy}_c{crash_policy}_"
                            f"v{vol_policy}_d{value_policy}_m{market_policy}"
                            )
                            trades.to_json(
                            ...
                            **counts,
                            ...
                            print(
                            f"[{combination:>3}/{total_combinations}] "
                            f"{frequency:<7} {heat_policy:<10} {crash_policy:<14} "
                            f"{vol_policy:<12} {value_policy:<12} {market_policy:<11} "
    ...
    summary.to_csv(output / "summary_seedforge_008.csv", index=False, encoding="utf-8-sig")
    period_summary = pd.DataFrame(period_summaries)
    period_summary.to_csv(output / "periods_seedforge_008.csv", index=False, encoding="utf-8-sig")
    print("\n상위 5개 조합(점수순)", flush=True)
    for rank, row in enumerate(summary.head(5).itertuples(index=False), 1):
        print(
            f"#{rank} 주기 {row.frequency} / 과열 {row.heat_policy} / "
            f"붕괴 {row.crash_policy} / 변동성 {row.vol_policy} / "
            f"거래대금 {row.value_policy} / 시장 {row.market_policy} | "
            f"CAGR {row.cagr:.2%}, MDD {row.mdd:.1%}, PF {row.trade_pf:.2f}, "
            f"거래수 {row.filled}, 점수 {row.score:.2f}",
            flush=True,
        )
    print(f"요약 저장: {output / 'summary_seedforge_008.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_008.csv'}", flush=True)
    independent = pd.DataFrame(
        independent_period_rows(summary, market, overbought, neutral, kospi_close)
    )
    independent.to_csv(output / "walkforward_seedforge_008.csv", index=False, encoding="utf-8-sig")
    print(f"독립 기간 재시뮬레이션 저장: {output / 'walkforward_seedforge_008.csv'}", flush=True)
    best = summary.iloc[0]
    best_periods = period_summary.loc[
        (period_summary["frequency"] == best["frequency"])
        & (period_summary["heat_policy"] == best["heat_policy"])
        & (period_summary["crash_policy"] == best["crash_policy"])
        & (period_summary["vol_policy"] == best["vol_policy"])
        & (period_summary["value_policy"] == best["value_policy"])
        & (period_summary["market_policy"] == best["market_policy"])
    ]
