print("SeedForge 020 파일 실행 시작", flush=True)
...
print("SeedForge 020 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_020_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 020의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 020 매수 필터 팩터 계산 중...", flush=True)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_020.py 를 끝까지 실행하세요.", flush=True)
        return
...
def add_validation_scores(summary: pd.DataFrame, period_summary: pd.DataFrame) -> pd.DataFrame:
    """전체 점수에 train/test 안정성 정보를 붙여 020 최종 2후보를 정렬한다."""
...
def analyze_exits() -> None:
    print("SeedForge 020 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_020.csv"
    if not summary_path.exists():
        print("summary_seedforge_020.csv가 없습니다. 먼저 python -u seedforge_020.py 를 실행하세요.", flush=True)
        return
...
def audit_end_of_test() -> None:
    ...
    print("SeedForge 020 end_of_test 코호트 감사", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_020.csv"
    if not summary_path.exists():
        print("summary_seedforge_020.csv가 없습니다. 먼저 python -u seedforge_020.py 를 실행하세요.", flush=True)
    ...
    cohort_path = output / "exit_reason_cohorts_seedforge_020.csv"
    worst_path = output / "end_of_test_worst_seedforge_020.csv"
...
def audit_entry_side(market: SimulationInputs, factors: EntryFactors) -> None:
    ...
    print("SeedForge 020 진입조건 코호트 감사", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_020.csv"
    if not summary_path.exists():
        print("summary_seedforge_020.csv가 없습니다. 먼저 python -u seedforge_020.py 를 실행하세요.", flush=True)
    ...
    audit_path = output / "entry_side_audit_seedforge_020.csv"
    cohort_path = output / "entry_side_cohorts_seedforge_020.csv"
...
def main() -> None:
    ...
    if "--audit-entry" in sys.argv:
        ...
        return
    refresh_cache = "--refresh-cache" in sys.argv
    print("SeedForge 020 진입 품질 × 시장국면 필터 검증 시작", flush=True)
    ...
    # 020은 매도규칙을 그대로 두고 시장국면 필터로 2018~2020 MDD가 줄어드는지만 검증한다.
    entry_presets = (
    ...
    summary.to_csv(output / "summary_seedforge_020.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(output / "periods_seedforge_020.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_020.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_020.csv'}", flush=True)
    best = summary.iloc[0]
