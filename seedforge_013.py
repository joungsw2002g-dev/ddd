print("SeedForge 013 파일 실행 시작", flush=True)
...
print("SeedForge 013 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_013_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 013의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def simulate(
    ...
    for i, date in enumerate(dates):
        ...
        for j in list(positions):
            ...
            if reason in {"neutral_partial_70", "neutral_loss_half"}:
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 013 매수 필터 팩터 계산 중...", flush=True)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_013.py 를 끝까지 실행하세요.", flush=True)
        return
...
def add_validation_scores(summary: pd.DataFrame, period_summary: pd.DataFrame) -> pd.DataFrame:
    """전체 점수에 train/test 안정성 정보를 붙여 013 최종 후보 순위를 만든다."""
...
def analyze_exits() -> None:
    print("SeedForge 013 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_013.csv"
    if not summary_path.exists():
        print("summary_seedforge_013.csv가 없습니다. 먼저 python -u seedforge_013.py 를 실행하세요.", flush=True)
        return
...
def main() -> None:
    ...
    print("SeedForge 013 RSI 다이버전스 최종 후보 재검증 시작", flush=True)
    ...
    print(
        ...
        "012에서 보완정책 실패 확인 후 base risk 고정 × 최종 매도 후보 재검증 시작",
        flush=True,
    )
    # 012에서 hold/trailing/partial 보완책이 모두 base보다 낮아졌으므로 risk는 base로 고정한다.
    # 013은 최종 매수 구조의 value 강도(dvchg_top20/top10)와 생존 매도정책만 재확인한다.
    entry_presets = (
        ...
        ("m_hnone_cr252_bottom20_vvol60_top20_ddvchg_top10_mkospi_ma200", "monthly", "none", "r252_bottom20", "vol60_top20", "dvchg_top10", "kospi_ma200"),
    )
    exit_policies = (
        "neutral_plus_overbought",
        "neutral_plus_overbought_delayed_be",
    ...
    summary.to_csv(output / "summary_seedforge_013.csv", index=False, encoding="utf-8-sig")
    period_summary.to_csv(output / "periods_seedforge_013.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_013.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_013.csv'}", flush=True)
    best = summary.iloc[0]
