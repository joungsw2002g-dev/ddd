print("SeedForge 009 파일 실행 시작", flush=True)
...
print("SeedForge 009 모듈 로딩 중...", flush=True)
...
D = Path("./data")
CACHE = D / "cache" / "seedforge_009_prepared.pkl"
PERIODS = (
...
@dataclass(frozen=True)
class EntryFactors:
    """SeedForge 009의 매수 후보 필터 계산을 전체 실행에서 1회 재사용한다."""
...
def simulate(
    ...
    neutral_enabled = exit_policy in {"neutral_only", "neutral_plus_overbought", "full_rsidiv9"}
    overbought_enabled = exit_policy in {"overbought_only", "neutral_plus_overbought", "full_rsidiv9"}
    ...
    for i, date in enumerate(dates):
        ...
        for j in list(positions):
            ...
            elif overbought_enabled and j in overbought_at.get(i, set()):
...
def prepare_entry_factors(market: SimulationInputs, kospi_close: pd.Series) -> EntryFactors:
    print("SeedForge 009 매수 필터 팩터 계산 중...", flush=True)
...
def summarize_trade_file(path: Path) -> None:
    print(f"\n{path}", flush=True)
    if not path.exists():
        print("파일이 없습니다. 먼저 python -u seedforge_009.py 를 끝까지 실행하세요.", flush=True)
        return
...
def analyze_exits() -> None:
    print("SeedForge 009 거래원장 청산사유 분석", flush=True)
    output = D / "results"
    summary_path = output / "summary_seedforge_009.csv"
    if not summary_path.exists():
        print("summary_seedforge_009.csv가 없습니다. 먼저 python -u seedforge_009.py 를 실행하세요.", flush=True)
        return
...
def main() -> None:
    ...
    print("SeedForge 009 RSI 다이버전스 매도규칙 분해 테스트 시작", flush=True)
    ...
    print(
        ...
        "상위 매수필터 × 매도정책 분해 시뮬레이션 시작",
        flush=True,
    )
    entry_presets = (
        ("w_r120_c10_v120_d10_mnone", "weekly", "r120_top10", "r252_bottom10", "vol120_top20", "dvchg_top10", "none"),
        ("m_none_c20_v60_d10_ma200", "monthly", "none", "r252_bottom20", "vol60_top20", "dvchg_top10", "kospi_ma200"),
        ("m_none_c20_v60_d20_ma200", "monthly", "none", "r252_bottom20", "vol60_top20", "dvchg_top20", "kospi_ma200"),
        ("m_r60_c20_v60_none_mnone", "monthly", "r60_top10", "r252_bottom20", "vol60_top20", "none", "none"),
        ("w_none_c10_v120_d20_mnone", "weekly", "none", "r252_bottom10", "vol120_top20", "dvchg_top20", "none"),
    )
    exit_policies = (
        "exit_none",
        "neutral_only",
    ...
    summary.to_csv(output / "summary_seedforge_009.csv", index=False, encoding="utf-8-sig")
    period_summary = pd.DataFrame(period_summaries)
    period_summary.to_csv(output / "periods_seedforge_009.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"요약 저장: {output / 'summary_seedforge_009.csv'}", flush=True)
    print(f"기간분리 저장: {output / 'periods_seedforge_009.csv'}", flush=True)
    best = summary.iloc[0]
